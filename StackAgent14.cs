...
public class StackAgent14 : Agent
{
    ...
    bool CheckGathered()
    {
        ...
        for (int i = 0; i < Stacked_Box_list.Count-1; i++)
        {
            ...
            {
                tempVariance += Mathf.Pow(Vector3.Distance(Stacked_Box_list[i].worldCenterOfMass, Stacked_Box_list[j].worldCenterOfMass), 2.0f);
            }
        }
        if (tempVariance <= boundary)
            gathered = true;
        else
            gathered = false;
        return gathered;
